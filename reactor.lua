...
function changetargetStrength(num, val)
	
	if val == 1 then
		targetStrength = targetStrength+num
	else
		targetStrength = targetStrength-num
	end
	updateReactorInfo()
...
function TargetStrength()
    ...
    MenuText = "Target Field Strength"

    clearMenuArea() -- Clear old buttons

    -- Define button data (Label, Value, Change Type)
    local buttonData = {
        {label = "+10%", value = 10, changeType = 1},   -- +10%
        {label = "+5%", value = 5, changeType = 1},     -- +5%
        {label = "+1%", value = 1, changeType = 1},     -- +1%
        {label = "-1%", value = 1, changeType = 0},     -- -1%
        {label = "-5%", value = 5, changeType = 0},     -- -5%
        {label = "-10%", value = 10, changeType = 0},   -- -10%
    }
    ...
    for _, data in ipairs(buttonData) do
        ...
        button.setButton(data.label, data.label, changetargetStrength, startX, buttonY, endX, buttonY + 2, data.value, data.changeType, colors.blue)
...
function buttonMain()
    ...
    local sLength3 = (sLength+14+(string.len("Shield"))+1)
    button.setButton("shield", "Shield", TargetStrength, sLength+14, 28, sLength3, 30, 0, 0, colors.blue)

    local sLength4 = (sLength+30+(string.len("Temp"))+1)
    button.setButton("temp", "Temp", MaxTemp, sLength+30, 28, sLength4, 30, 0, 0, colors.blue)
